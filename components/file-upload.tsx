    ...
    <div className={`space-y-4 ${className}`}>
      ...
      <div
        className={`
          ...
          ${isDragActive 
            ? theme?.tenantSlug === 'siga-turismo' ? 'border-siga-green bg-siga-green/10 scale-105' : 'border-primary bg-primary/10 scale-105'
            : theme?.tenantSlug === 'siga-turismo' ? 'border-siga-green/50 hover:border-siga-green bg-card/50 hover:bg-siga-green/5' : 'border-border hover:border-border/70 bg-card/50'
          }
      ...
      >
        ...
        <div className="flex flex-col items-center gap-4">
          <div className={`
            ...
            ${isDragActive 
              ? theme?.tenantSlug === 'siga-turismo' ? 'bg-siga-green/20' : 'bg-primary/20'
              : theme?.tenantSlug === 'siga-turismo' ? 'bg-siga-green/10' : 'bg-muted/50'
            }
          `}>
            <Upload className={`h-6 w-6 ${
              isDragActive 
                ? theme?.tenantSlug === 'siga-turismo' ? 'text-siga-green' : 'text-primary'
                : theme?.tenantSlug === 'siga-turismo' ? 'text-siga-green' : 'text-muted-foreground'
            }`} />
          </div>
          
          <div>
            <h3 className={`text-lg font-semibold mb-2 ${
              theme?.tenantSlug === 'siga-turismo' ? 'text-siga-green' : 'text-foreground'
            }`}>
              ...
              <div key={index} className="space-y-3">
                {result.success && result.data ? (
                  <div className={`rounded-lg p-4 ${
                    theme?.tenantSlug === 'fema-electricidad' ? 'bg-fema-blue/10 border border-fema-blue/20' :
                    theme?.tenantSlug === 'siga-turismo' ? 'bg-siga-green/10 border border-siga-green/20' :
                    'bg-primary/10 border border-primary/20'
                  }`}>
                    <div className="flex items-start gap-3">
                      <CheckCircle className={`h-5 w-5 mt-0.5 ${
                        theme?.tenantSlug === 'fema-electricidad' ? 'text-fema-blue' :
                        theme?.tenantSlug === 'siga-turismo' ? 'text-siga-green' :
                        'text-primary'
                      }`} />
                      <div className="flex-1 space-y-2">
                              ...
                              {result.data.sheetDetails.map((sheet, i) => (
                                <div key={i} className="flex items-center justify-between bg-muted/20 rounded px-3 py-2">
                                  ...
                                  <Badge variant="outline" className={`${
                                    theme?.tenantSlug === 'fema-electricidad' ? 'bg-fema-blue/20 text-fema-blue border-fema-blue/30' :
                                    theme?.tenantSlug === 'siga-turismo' ? 'bg-siga-green/20 text-siga-green border-siga-green/30' :
                        ...
                        {result.data.errors.length > 0 && (
                          <div className="mt-3">
                            <h5 className={`font-medium mb-2 ${
                              theme?.tenantSlug === 'fema-electricidad' ? 'text-fema-yellow' :
                              theme?.tenantSlug === 'siga-turismo' ? 'text-siga-yellow' :
                              'text-secondary'
                            }`}>
                              Advertencias ({result.data.errors.length}):
                            </h5>
                            <ul className={`space-y-1 text-sm ${
                              theme?.tenantSlug === 'fema-electricidad' ? 'text-fema-yellow/80' :
                              theme?.tenantSlug === 'siga-turismo' ? 'text-siga-yellow/80' :
                              'text-secondary/80'
                            }`}>
                        ...
                        {result.data.parsedData.length > 0 && (
                          <div className="mt-3">
                            ...
                            <div className={`bg-muted/30 rounded border overflow-x-auto ${
                              theme?.tenantSlug === 'fema-electricidad' ? 'border-fema-blue/20' :
                              theme?.tenantSlug === 'siga-turismo' ? 'border-siga-green/20' :
                              'border-primary/20'
                            }`}>
                                ...
                                <tbody>
                                  {result.data.parsedData.slice(0, 3).map((row, i) => (
                                    <tr key={i} className={`border-b ${
                                      theme?.tenantSlug === 'fema-electricidad' ? 'border-fema-blue/10' :
                                      theme?.tenantSlug === 'siga-turismo' ? 'border-siga-green/10' :
                                      'border-primary/10'
                                    }`}>
