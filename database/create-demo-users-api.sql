-- Create demo users for each tenant (API-compatible version)
-- This version works with the trigger by providing proper metadata
...
-- The trigger will automatically create user_profiles if metadata is correct
DO $$
DECLARE
...
BEGIN
  ...
  -- Create identities for auth.users (if they don't exist)
  ...
  -- If the trigger didn't create user_profiles automatically, create them manually
  -- This handles cases where the trigger might fail or not work as expected
  
  -- First check if profiles were created by trigger
  IF NOT EXISTS (SELECT 1 FROM public.user_profiles WHERE email = 'ceo@stratix-demo.com') THEN
    ...
    SELECT 
      ...
      CASE 
        WHEN u.raw_user_meta_data->>'role' = 'CEO' THEN 'Executive'
        WHEN u.raw_user_meta_data->>'role' = 'Admin' THEN 'Administration'
        ELSE 'General'
      END,
      true,
      NOW(),
