---
description: "A detailed workflow for reviewing Pull Requests in the consultant-copilot project, utilizing the gh CLI."
author: "Consultant Copilot Team"
version: "1.0"
tags: ["pr review", "github", "consultant-copilot", "firebase", "nextjs"]
---
You have access to the `gh` terminal command for reviewing PRs in the consultant-copilot repository.
...
Let's walk through an example of reviewing a PR that adds a new client integration feature.
...
```xml
# Examine Firebase Functions for backend changes
<read_file>
...
<read_file>
<path>components/client-card.tsx</path>
</read_file>

# Review integration architecture if relevant
<read_file>
<path>instructins/third_party_software.md</path>
</read_file>
```

## Step 3: Analyze the Changes

From the PR information and code examination, check for:

1. **Firebase Integration**: Proper use of Firebase services (Auth, Firestore, Functions)
2. **Next.js Best Practices**: App Router usage, client/server components
3. **TypeScript**: Type safety and interface definitions
4. **Security**: No exposed credentials, proper authentication checks
5. **Spanish Localization**: UI text properly translated
6. **Architecture Compliance**: Follows patterns in CLAUDE.md
7. **Testing**: Emulator compatibility, error handling

## Step 4: Ask for User Confirmation

```xml
<ask_followup_question>
<question>Based on my review of PR #<PR-number>, I recommend [approving/requesting changes]. Here's my assessment:
...
cat << EOF | gh pr review <PR-number> --approve --body-file -
This PR looks good! The implementation follows the consultant-copilot architecture well.

I particularly like:
1. The proper Firebase integration patterns
2. The use of existing shadcn/ui components
3. The TypeScript type safety throughout
4. The consideration for Spanish localization
