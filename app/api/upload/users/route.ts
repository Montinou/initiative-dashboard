...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check permissions (only Admin and CEO can import users)
    if (!['Admin', 'CEO'].includes(userProfile.role)) {
      return NextResponse.json(
        { error: 'Insufficient permissions. Only Admin and CEO can import users.' },
        { status: 403 }
      );
    }
    ...
    return NextResponse.json(
      { 
        error: 'Failed to process user import',
...
export async function GET(request: NextRequest) {
  try {
    ...
    // Get preview
    const preview = await UserImportProcessor.getPreview(
      fileBuffer,
      contentType || 'text/csv'
    );
